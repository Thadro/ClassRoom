...
function onDeleteClass()
{
    ...
    let class_Target = $('.delete-class-button-target').attr('id');
    ...
    //PROBLEME: SI ON SUPPRIME UNE CLASSE CELA NE SUPPRIME PAS LES DONNEES UTILISATEURS DES ELEVES CONTENU DANS USER LIST
}
...
//Fonction 4: Permet de rajouter un utilisateur dans la base de donnée
...
function onAddUser(event)
{
    ...
    //Probleme: On ne peut pas accéder à la liste des utilisateurs dans l'autentification si on les enregistre par classe
}


//Fonction 5: Permet de supprimer un utilisateur de la base de donnée
...
//Fonction 6: Permet de modifier les information d'un utilisateur dans la base de donnée
function onModifUser(event)
{
    ...
    console.log(attribute, user_Selected);
...
//Partie 1: Affichage des données
...
function showPlanning(class_Selected)
{
    database.ref('classroom/' +class_Selected).on('value', function(snapshot) {
        ...
        snapshot.forEach(function(item){
            ...
            {
                $(`td[class*=${classroom.date}]`).css('background-color', 'red');
            ...
            else
            {
                ...
                $(table_Target).css('background-color', 'red');
                $(table_Target).append(`
                <div class="classroom-title">
                    <button class="delete-classroom-button" id=${classroom.classroom_Id}><i class="fas fa-times"></i></button>
                    <p>${classroom.title}</p>
...
//Partie 2: Modification des données
...
//Fonction 3: Permet de supprimer un cours
